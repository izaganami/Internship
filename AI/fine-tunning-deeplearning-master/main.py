...
checkpoint = ModelCheckpoint('checkpoints/', monitor='loss', verbose=1,
...
# Labels for possible configurations
...
# Load all the labeled images and store them in 'data' and their labels in 'labels'.
...
# Augmentation of the data ( File data_preparation )
...
# Prepare the ResNet50 base model .
baseModel = ResNet50(weights="imagenet", include_top=False, input_tensor=Input(shape=(224, 224, 3)))
headModel = baseModel.output
##up to here pretrained resnet model

# NN operations ( Hyperparameters to adjust ).
...
# Compile the model with SGD optimzer and accuracy as metric.
model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=["accuracy"])

# Fit the model.
print("[INFO] training head...")
print(len(trainX))
H = model.fit(trainAug.flow(trainX, trainY, batch_size=400),validation_data=valAug.flow(testX, testY),validation_steps=len(testX) // 32, epochs=args["epochs"] , callbacks=[checkpoint] )
